...
function showLogin() {
    // Show existing user login modal
    ...
    try {
        ...
        if (users.length > 0) {
            ...
            if (user.subscription_status === 'trial' && trialEnd > now) {
                alert(`Merhaba ${user.name}! Trial süreniz devam ediyor (${Math.ceil((trialEnd - now) / (1000 * 60 * 60 * 24))} gün kaldı). WhatsApp'tan mesaj göndermeye devam edebilirsiniz.`);
            } else if (user.subscription_status === 'active') {
                alert(`Merhaba ${user.name}! Aktif aboneliğiniz var. WhatsApp'tan mesaj göndermeye devam edebilirsiniz.`);
            } else {
                alert(`Merhaba ${user.name}! Trial süreniz dolmuş veya aboneliğiniz pasif. Ödeme yapmak için yönlendiriliyorsunuz.`);
                // TODO: Redirect to payment page when ready
                alert('Ödeme sayfası henüz hazır değil. Lütfen info@satinalmacopilot.com ile iletişime geçin.');
            }
        } else {
            alert('Bu numara ile kayıtlı kullanıcı bulunamadı. Lütfen kayıt olun.');
            scrollToForm();
        }
    } catch (error) {
        console.error('User check error:', error);
        alert('Kullanıcı kontrolü yapılırken hata oluştu. Lütfen tekrar deneyin.');
    }
}
    ...
    try {
        ...
        if (existingUsers.length > 0) {
            ...
            if (existingUser.trial_used) {
                alert('Bu numara ile daha önce ücretsiz deneme kullanılmış. Ödeme yaparak devam edebilirsiniz.');
                // TODO: Redirect to payment when ready
                alert('Ödeme sayfası henüz hazır değil. Lütfen info@satinalmacopilot.com ile iletişime geçin.');
                resetButton();
                return;
            } else {
                alert('Bu numara ile zaten kayıt bulunuyor. Mevcut hesabınızı kullanabilirsiniz.');
                resetButton();
                return;
            }
        }
